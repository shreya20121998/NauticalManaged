    ...
    return BaseController.extend("com.ingenx.nauti.createvoyage.controller.VoyageApproval", {
      ...
      VoyageValueHelpClose: async function (evt) {
        ...
        if (!aSelectedContexts || !aSelectedContexts.length) {
          ...
          evt.getSource().setNoDataText("Loading ...")
          return;
        }
      ...
      createDynamicColumns1: function (approvers) {
        let oTable = this.getView().byId("approvalTable");
        let oColumnTemplate = new sap.m.Text(); // Column template for text
      ...
      onVoyageSearch1: function (oEvent) {
        ...
        // Attach an event listener to handle the change event after filtering
        oBinding.attachEventOnce("change", function () {
          if (!oSource.getItems().length) {
            oSource.setNoDataText("No data found");
          }
        });
